                        ...
                        <div class="wrap-logo">
                            ...
                            <div class="wrap-right-title">
                                <div class="wrap-right-img">
                                    <img alt="서브타이틀" class="sub-title_img"  src="../images/sub_title/sc_qna_title.png">
                                </div>
            ...
            <div>
                <div  class="wrap-right__option">
                    <form action="<%= cur_path %>/admin/work.jsp" id="search" method="post">
                    	<input type="hidden" name="search_page" value="../admin/ad_admin_qna.jsp">
                ...
                <div>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-header">
                        ...
                        ArrayList<qnaVO> list = qnaDAO.getList(pageNumber, search_info, select_option);
                        for(int i=0;i<list.size();i++){
						%>
						<c:set var="isReply" value="<%= list.get(i).getQnA_reply() %>"/>
						<tr>
						...
						<td><a onclick="subForm('../ad_action/QnA_action2.jsp','action<%=tmp%>')"><%=list.get(i).getQnA_title() %>
						<c:if test="${isReply ne null}">
							<img alt="답변달린글" src="https://biztax.hscity.go.kr/hwaseong/image/ic_reply.gif">
						</c:if>
						</a></td>
						
						<td><%=list.get(i).getQnA_viewnum() %></td>
						
						<td><%=list.get(i).getQnA_regisdate() %></td>
					...
					<input type="hidden" name= "select_option" value="<%=select_option%>" >
						...
						<a onclick="subForm('../ad_action/QnA_page_action.jsp','page<%=pageNumber - 1%>')" style="float:left;">이전</a>
					<%
						} if(qnaDAO.nextPage(pageNumber + 1, search_info, select_option)) {
					%>
					<form action="<%=cur_path %>/admin/work.jsp" name="page<%=pageNumber + 1%>" method="POST" class="page<%=pageNumber + 1%>">
					<input type="hidden" name= "pageNumber" value="<%=pageNumber + 1%>" >
					<input type="hidden" name= "search_info" value="<%=search_info%>" >
					<input type="hidden" name= "select_option" value="<%=select_option%>" >
						...
						<a onclick="subForm('../ad_action/QnA_page_action.jsp','page<%=pageNumber + 1%>')" style="float:left;">다음</a>
					<%
						}
					%>
					</div>
